...
function validateRecordValues(t, data, file) {
    ...
    Object.entries(data.records).forEach(([key, value]) => {
        ...
        if (["CNAME", "URL"].includes(key)) {
            ...
            if (key === "CNAME") {
                ...
                t.true(value !== `${subdomain}.dev.wales`, `${file}: ${key} cannot point to itself`);
                t.true(value !== "dev.wales", `${file}: ${key} cannot point to dev.wales`);
                ...
                t.false(urlHost === `${subdomain}.dev.wales`, `${file}: ${key} cannot point to itself`);
            }
        }
    ...
    if (data.redirect_config) {
        ...
        customPaths.forEach((customPath, idx) => {
            ...
            t.false(urlHost === `${subdomain}.dev.wales`, `${urlMessage} cannot point to itself at index ${idx}`);
        });
    }
}
